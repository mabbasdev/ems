...
export default function AdminDashboard(props) {
    ...
    // Use persistent hook instead of raw useState.
    // `tasks` automatically reads from LocalStorage on mount, and `setTasks` writes back to LocalStorage!
    ...
    // Save handler triggered on Form Submit
    const handleSaveTask = (e) => {
        e.preventDefault(); // Prevent standard page reloads on form submission
        if (!newTask.title.trim()) return; // Prevent creating blank tasks

        if (editingTask) {
            // SCENARIO A: EDITING AN EXISTING TASK
            // Map over current tasks and update fields for the matching task ID
            const updatedTasks = tasks.map((t) =>
                t.id === editingTask.id
                    ? {
                        ...
                        // Generate avatar initials dynamically (e.g. "Muhammad Abbas" -> "MA")
            ...
            // Trigger state update -> auto-writes to LocalStorage
            setTasks(updatedTasks);
        } else {
            // SCENARIO B: CREATING A BRAND NEW TASK
            const createdTask = {
                id: `TSK-${101 + tasks.length}`, // Auto-increment tracking ID based on array length
                title: newTask.title,
                assignee: newTask.assignee,
                assigneeAvatar: newTask.assignee
                    .split(" ")
                    ...
                    .join(""),
                priority: newTask.priority,
                status: "In Progress", // Default status for new admin assignments
                dueDate: newTask.dueDate || "2026-07-28",
                department: newTask.department,
                description: newTask.description || "",
            };

            // Prepend new task to state -> auto-writes to LocalStorage & alerts any open Employee tab
    ...
    return (
        <div className="min-h-screen w-full bg-slate-50 dark:bg-slate-950 text-slate-800 dark:text-slate-100 font-sans transition-colors duration-300 flex">
            ...
            >
                ...
                {/* User Info & Embedded Profile Dropdown (Desktop Sidebar) */}
                <div className="pt-4 border-t border-slate-100 dark:border-slate-800 relative">
                    <button
                        ...
                        className={`w-full flex items-center ${isSidebarCollapsed ? "justify-center" : "justify-between px-2"
                            } py-2 rounded-xl hover:bg-slate-100 dark:hover:bg-slate-800/80 transition-colors text-left`}
                    >
                        ...
                        {!isSidebarCollapsed && (
                            <ChevronUp
                                className={`w-4 h-4 text-slate-400 transition-transform duration-200 ${desktopProfileOpen ? "rotate-180" : ""
                                    }`}
                            />
                        )}
                    </button>

                    {/* Desktop Sidebar Popover Menu */}
                    {desktopProfileOpen && (
                        <>
                            <div
                            ...
                            />
                            <div className="absolute bottom-full left-0 mb-2 w-56 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl shadow-xl p-2 z-50 animate-in fade-in slide-in-from-bottom-2 duration-200 space-y-1">
                                ...
                                <button
                                    ...
                                    className="w-full flex items-center gap-2.5 px-3 py-2 text-xs font-medium text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-xl transition-colors"
                                >
                                    <Sliders className="w-4 h-4 text-slate-400" /> System Settings
                                </button>
            ...
            {mobileSidebarOpen && (
                <div className="fixed inset-0 z-50 lg:hidden">
                    ...
                    <aside className="absolute top-0 left-0 bottom-0 w-64 bg-white dark:bg-slate-900 p-4 flex flex-col justify-between z-50">
                        ...
                        {/* Mobile Sidebar Profile Option */}
                        <div className="pt-4 border-t border-slate-100 dark:border-slate-800 relative">
                            <button
                                onClick={() => setMobileProfileOpen(!mobileProfileOpen)}
                                className="w-full flex items-center justify-between p-2 rounded-xl bg-slate-50 dark:bg-slate-950/60 border border-slate-200 dark:border-slate-800"
                            >
                                <div className="flex items-center gap-2.5">
                                    <div className="h-8 w-8 rounded-full bg-slate-900 dark:bg-emerald-600 text-white font-bold text-xs flex items-center justify-center shrink-0">
                                        MA
                                    </div>
                                ...
                                <ChevronUp
                                    className={`w-4 h-4 text-slate-400 transition-transform duration-200 ${mobileProfileOpen ? "rotate-180" : ""
                                        }`}
                                />
                            </button>

                            {mobileProfileOpen && (
                                <div className="mt-2 w-full bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-2 shadow-lg space-y-1">
                                    <button
                                        onClick={() => {
                                            setActiveNav("settings");
                                            setMobileSidebarOpen(false);
                                    ...
                                    >
                                        <User className="w-3.5 h-3.5 text-slate-400" /> Profile Options
                                    </button>
                                    <button
                                        onClick={props.handleLogout}
